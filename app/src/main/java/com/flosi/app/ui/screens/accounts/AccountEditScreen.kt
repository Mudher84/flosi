...
@Composable fun AccountEditScreen(onBack:()->Unit){
 ...
 FlosiPage("إضافة حساب","حساب أو محفظة بعملة مستقلة",onBack){
  ...
  Text("نوع الحساب")
  Row(horizontalArrangement=Arrangement.spacedBy(6.dp)){listOf("cash" to "نقدي","bank" to "مصرف","wallet" to "محفظة").forEach{(k,l)->FilterChip(type==k,{type=k},{Text(l)})}}
  Text("عملة الحساب")
  ...
  Button(onClick={vm.add(name,type,balance.toLongOrNull()?:0,currency);onBack()},enabled=name.isNotBlank(),modifier=Modifier.fillMaxWidth()){Text("حفظ")}
 }
}
