...
@Composable fun AccountDetailScreen(id:Long,onBack:()->Unit,onTransfer:()->Unit){
 ...
 FlosiPage(account?.name ?: flosiText("account"),localizedLegacyText("تفاصيل الحساب"),onBack){
  account?.let{a->CardBox{Metric(localizedLegacyText("الرصيد الحالي"),moneyText(a.currentBalance,a.currency),FlosiPurple);Text("${a.type} • ${a.currency}",color=FlosiMuted)}}
  CardBox{ActionRow(localizedLegacyText("تحويل من/إلى الحساب"),localizedLegacyText("إدارة التحويلات"),onClick=onTransfer)}
 }
}
