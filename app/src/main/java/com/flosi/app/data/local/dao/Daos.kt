...
interface PersonDao {
    ...
    @Query("UPDATE people SET currentBalance = currentBalance + :delta WHERE id=:id")
    suspend fun adjustBalance(id: Long, delta: Long)

    @Query("SELECT * FROM people WHERE archived=0 AND (name LIKE '%' || :q || '%' OR phone LIKE '%' || :q || '%') ORDER BY name LIMIT 40")
...
interface CategoryDao {
    ...
    @Query("SELECT * FROM categories WHERE archived=0 AND name LIKE '%' || :q || '%' LIMIT 40")
    suspend fun search(q: String): List<CategoryEntity>
}
...
interface TransactionDao {
    @Query("""
        SELECT t.id,t.kind,t.amount,t.title,t.note,t.occurredAt,
               a.name AS accountName,p.name AS personName,c.name AS categoryName
        FROM transactions t
    ...
    @Query("""
        ...
        FROM transactions
        WHERE deleted=0 AND occurredAt BETWEEN :from AND :to
    """)
...
interface CommitmentDao {
    @Query("SELECT * FROM commitments WHERE active=1 ORDER BY dueAt")
    fun observeActive(): Flow<List<CommitmentEntity>>
    @Query("SELECT * FROM commitments WHERE id=:id LIMIT 1")
    fun observe(id:Long): Flow<CommitmentEntity?>
...
@Dao
interface BudgetDao {
    @Query("SELECT * FROM budgets WHERE active=1 ORDER BY periodEnd")
    fun observeActive(): Flow<List<BudgetEntity>>
    @Query("SELECT * FROM budgets WHERE id=:id LIMIT 1")
    fun observe(id:Long): Flow<BudgetEntity?>
...
@Dao
interface GoalDao {
    @Query("SELECT * FROM goals WHERE active=1 ORDER BY targetDate")
