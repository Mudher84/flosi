...
#addModal .txMoreGrid{display:grid;grid-template-columns:1fr 1fr;gap:8px;align-items:start}#addModal .txMoreGrid .field{margin:6px 0;min-width:0}
...
#addModal .txPillSelect.open{z-index:300}
#addModal .txPillMenu{position:relative;z-index:301;display:none;margin-top:6px;padding:7px;background:#fff;border:1px solid #e8e0f0;border-radius:20px;box-shadow:0 18px 42px rgba(52,35,72,.16);overflow:hidden}.txPillSelect.open .txPillMenu{display:grid;gap:4px}
...
function enhancePillSelect(sel){
 if(!sel)return;
 if(sel.dataset.pill==='1'){
  const existing=sel.nextElementSibling;if(existing&&existing.classList.contains('txPillSelect'))return;
  delete sel.dataset.pill;sel.classList.remove('txPillNative');
 }
 ...
 sync();
 btn.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();const willOpen=!wrap.classList.contains('open');document.querySelectorAll('#addModal .txPillSelect.open,#addModal .txTypeCapsule.open').forEach(x=>x.classList.remove('open'));wrap.classList.toggle('open',willOpen);btn.setAttribute('aria-expanded',String(willOpen));if(willOpen)wrap.scrollIntoView({block:'nearest',behavior:'smooth'})});
 menu.querySelectorAll('.txPillOption').forEach(o=>o.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();sel.selectedIndex=Number(o.dataset.i);sel.dispatchEvent(new Event('change',{bubbles:true}));wrap.classList.remove('open');btn.setAttribute('aria-expanded','false');sync()}));
