...
let openPortal=null;
function closePortal(){if(openPortal){openPortal.remove();openPortal=null}document.querySelectorAll('#addModal .txPillSelect.open,#addModal .txTypeCapsule.open').forEach(x=>x.classList.remove('open'))}
...
#addModal .txTypeCapsule,#addModal .txPillSelect{position:relative!important;overflow:visible!important}
#addModal .field,#addModal .txMoreGrid,#addModal .txMorePanel{overflow:visible!important}
...
.flosiPillPortal{position:fixed!important;z-index:2147483646!important;display:grid!important;gap:4px;padding:7px;background:#fff;border:1px solid #e8e0f0;border-radius:20px;box-shadow:0 18px 42px rgba(52,35,72,.22);box-sizing:border-box;direction:rtl;max-height:260px;overflow:auto!important}
.flosiPillPortal .txPillOption{width:100%;min-height:42px;border:0;border-radius:14px;background:#fff;color:#17131f;display:grid;grid-template-columns:28px 1fr 28px;align-items:center;padding:0 9px;font:inherit;cursor:pointer;text-align:start}.flosiPillPortal .txPillOption:hover{background:#faf7ff}.flosiPillPortal .txPillOption.active{background:linear-gradient(135deg,#f1eaff,#faf7ff);color:#6334ce;font-weight:700}
.flosiPillPortal .txPillCheck{display:grid;place-items:center;color:#6d36de;font-size:15px}.flosiPillPortal .txPillOptionText{padding-inline:3px}
...
function enhancePillSelect(sel){
 ...
 wrap.innerHTML=`<button type="button" class="txPillButton" aria-haspopup="listbox" aria-expanded="false"><span class="txPillSpacer"></span><span class="txPillValue"></span><span class="txPillChevron">⌄</span></button>`;
 sel.insertAdjacentElement('afterend',wrap);
 const btn=$('.txPillButton',wrap),value=$('.txPillValue',wrap);
 const sync=()=>{const i=sel.selectedIndex<0?0:sel.selectedIndex;value.textContent=sel.options[i]?.textContent||''};sync();
 const open=()=>{
   closePortal();wrap.classList.add('open');btn.setAttribute('aria-expanded','true');
   const r=btn.getBoundingClientRect();const menu=document.createElement('div');menu.className='flosiPillPortal';menu.setAttribute('role','listbox');
   const roomBelow=window.innerHeight-r.bottom;const estimated=Math.min(260,Math.max(52,sel.options.length*46+14));const above=roomBelow<estimated&&r.top>roomBelow;
   menu.style.left=Math.max(8,r.left)+'px';menu.style.width=Math.max(120,r.width)+'px';
   if(above){menu.style.bottom=(window.innerHeight-r.top+6)+'px'}else{menu.style.top=(r.bottom+6)+'px'}
   [...sel.options].forEach((op,i)=>{const b=document.createElement('button');b.type='button';b.className='txPillOption'+(i===sel.selectedIndex?' active':'');b.setAttribute('role','option');b.setAttribute('aria-selected',String(i===sel.selectedIndex));b.innerHTML=`<span class="txPillCheck">${i===sel.selectedIndex?'✓':''}</span><span class="txPillOptionText">${esc(op.textContent)}</span><span></span>`;b.onclick=e=>{e.preventDefault();e.stopPropagation();sel.selectedIndex=i;sel.dispatchEvent(new Event('change',{bubbles:true}));sync();closePortal();btn.setAttribute('aria-expanded','false')};menu.appendChild(b)});
   document.body.appendChild(menu);openPortal=menu;
 };
 btn.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();if(wrap.classList.contains('open')){closePortal();btn.setAttribute('aria-expanded','false')}else open()});
 sel.addEventListener('change',sync);
}
function build(){
 installStyle();
 const sel=$('#txType');if(!sel)return;
 if(sel.dataset.capsule!=='1'&&!$('#addModal .txTypeCapsule')){
